# BDG-King - Betting Game Backend API
...
- `GET /api/bets/user` - Get user's bet history (requires auth)

## Database Schema

- **users**: User accounts with balance and admin flag
- **deposits**: Deposit requests (pending/approved/rejected)
- **withdraws**: Withdrawal requests (pending/processed/rejected)
...
The default secret is for development only. Set a strong secret key in production environment.
